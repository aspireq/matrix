...
//        }
    ...
    public function activate_user($user_id, $activation_token = FALSE, $verify_token = TRUE, $clear_token = TRUE) {
        ...
        $email_data = array('username' => $userinfo->uacc_username);
        $this->flexi_auth->send_email($userinfo->email, 'Account Verified', 'activation_confirmation.php', $email_data);
        return $this->db->affected_rows() == 1;
    }
    ...
    public function forgotten_password($identity) {
        ...
        if ($this->auth->auth_security['expire_forgotten_password'] > 0) {
            ...
            $this->db->set($this->auth->tbl_col_user_account['forgot_password_expire'], $this->database_date_time($expire_time));
        }
    ...
    private function set_login_sessions($user, $logged_in_via_password = FALSE) {
        ...
        $this->auth->session_data[$this->auth->session_name['group']] = array($group->{$this->auth->database_config['user_group']['columns']['id']} => $group->{$this->auth->database_config['user_group']['columns']['name']});
...
//		);
        ...
        $sql_insert = array(
            'usess_uacc_fk' => $user_id,
            'usess_token' => $session_token,
            'usess_login_date' => $this->database_date_time()
        );

        $this->db->insert('user_login_sessions', $sql_insert);
    ...
    private function login_attempts_exceeded($identity) {
        ...
        if ($query->num_rows() == 1) {
            ...
            else if ($this->database_date_time($this->auth->auth_security['login_attempt_time_ban'], $failed_login_date, TRUE) < $this->database_date_time(FALSE, FALSE, TRUE)) {
                return FALSE;
            }
        }
    ...
    public function validate_recaptcha() {
        ...
        $response = recaptcha_check_answer(
                $this->auth->auth_security['recaptcha_private_key'], $this->input->ip_address(), $this->input->post('recaptcha_challenge_field'), $this->input->post('recaptcha_response_field')
        );
